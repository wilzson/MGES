...
class BLSTM_GAT_CRF(nn.Module):
    def __init__(self, data, args):
        ...
        self.gaz_embeddings = nn.Embedding(data.gaz_alphabet.size(), self.gaz_emb_dim)  # 创建词典的向量
        self.char_embeddings = nn.Embedding(data.char_alphabet.size(), self.char_emb_dim)  # 创建字符的向量
        self.biword_embeddings = nn.Embedding(data.biword_alphabet.size(), self.biword_emb_dim)  # 创建双词的向量
        if data.pretrain_char_embedding is not None:
            self.char_embeddings.weight.data.copy_(torch.from_numpy(data.pretrain_char_embedding))  # 获取预训练的字符向量的参数
        else:
            self.char_embeddings.weight.data.copy_(
                torch.from_numpy(self.random_embedding(data.char_alphabet.size(), self.char_emb_dim)))  # 随机参数
        # fzf 双词结构
        if args.use_biword:
            if data.pretrain_biword_embedding is not None:
                self.biword_embeddings.weight.data.copy_(
                    torch.from_numpy(data.pretrain_biword_embedding))  # 获取预训练的双词向量的参数
            else:
                self.biword_embeddings.weight.data.copy_(
                    torch.from_numpy(self.random_embedding(data.biword_alphabet.size(), self.biword_emb_dim)))  # 随机参数
        ...
        if args.fix_gaz_emb:
            self.gaz_embeddings.weight.requires_grad = False  # 需要梯度计算？
        else:
            self.gaz_embeddings.weight.requires_grad = True

        # fzf 双词结构
        char_feature_dim = self.char_emb_dim
        ...
        if args.use_biword:
            self.lstm = nn.LSTM(char_feature_dim, lstm_hidden, num_layers=self.lstm_layer, batch_first=True,
                                bidirectional=self.bilstm_flag)
        else:
            self.lstm = nn.LSTM(self.char_emb_dim, lstm_hidden, num_layers=self.lstm_layer, batch_first=True,
                                bidirectional=self.bilstm_flag)
        self.hidden2hidden = nn.Linear(self.hidden_dim, crf_input_dim)
        self.gat_1 = GAT(self.hidden_dim, args.gat_nhidden, crf_input_dim, args.dropgat, args.alpha, args.gat_nhead,
                         args.gat_layer)
        self.gat_2 = GAT(self.hidden_dim, args.gat_nhidden, crf_input_dim, args.dropgat, args.alpha, args.gat_nhead,
                         args.gat_layer)
        self.gat_3 = GAT(self.hidden_dim, args.gat_nhidden, crf_input_dim, args.dropgat, args.alpha, args.gat_nhead,
                         args.gat_layer)
        self.gat_4 = GAT(self.hidden_dim, args.gat_nhidden, crf_input_dim, args.dropgat, args.alpha, args.gat_nhead,
                         args.gat_layer)
        ...
        if self.strategy == "v":
            ...
            self.weight5 = nn.Parameter(torch.ones(crf_input_dim))
        ...
        # fzf 双词结构的dropout
        self.dropbiword = nn.Dropout(args.dropbiword)
        self.droplstm = nn.Dropout(args.droplstm)
        self.gaz_dropout = nn.Dropout(args.gaz_dropout)
        self.sdp_dropout = nn.Dropout(args.sdp_dropout)
    ...
    def to_cuda(self):
        ...
        self.gat_4 = self.gat_4.cuda()
        self.hidden2hidden = self.hidden2hidden.cuda()
        self.gaz_dropout = self.gaz_dropout.cuda()
        self.dropout = self.dropout.cuda()
        self.dropbiword = self.dropbiword.cuda()
        self.droplstm = self.droplstm.cuda()
        # fzf
        self.dropbiword = self.dropbiword.cuda()
        self.sdp_dropout = self.sdp_dropout.cuda()
        if self.strategy in ["v", "n"]:
            ...
            self.weight5.data = self.weight5.data.cuda()
        else:
    ...
    # def _get_lstm_features(self, batch_char, batch_biword, batch_len):
    def _get_lstm_features(self, batch_char, batch_len):
        embeds = self.char_embeddings(batch_char)
        # begin 添加biword信息， 合并最后一维信息
        embeds = self.dropout(embeds)
        # biword_embs = self.biword_embeddings(batch_biword)
        # biword_embs = self.dropbiword(biword_embs)
        # embeds = torch.cat([embeds,biword_embs],dim=-1)
        # end
        embeds_pack = pack_padded_sequence(embeds, batch_len, batch_first=True)  # 压缩填充张量 batch 是否在第一位
    ...
    def _get_crf_feature(self, batch_char, batch_biword, batch_len, gaz_list, b_graph, c_graph, l_graph, s_graph,
                         sdp_list):
        gaz_feature = self.gaz_embeddings(gaz_list)  # 匹配词语对应的特征
        gaz_feature = self.gaz_dropout(gaz_feature)
        sdp_feature = self.gaz_embeddings(sdp_list)  # 语义依存对应特征
        sdp_feature = self.sdp_dropout(sdp_feature)  #
        # lstm_feature = self._get_lstm_features(batch_char,batch_biword, batch_len.cpu())
        lstm_feature = self._get_lstm_features(batch_char, batch_len.cpu())
        max_seq_len = lstm_feature.size()[1]
        gat_input = torch.cat((lstm_feature, gaz_feature), dim=1)
        sdp_input = torch.cat((lstm_feature, sdp_feature), dim=1)
        gat_feature_1 = self.gat_1(gat_input, c_graph)  #
        gat_feature_1 = gat_feature_1[:, :max_seq_len, :]
        gat_feature_2 = self.gat_2(sdp_input, s_graph)
        gat_feature_2 = gat_feature_2[:, :max_seq_len, :]
        gat_feature_3 = self.gat_3(gat_input, l_graph)
        gat_feature_3 = gat_feature_3[:, :max_seq_len, :]
        gat_feature_4 = self.gat_4(gat_input, l_graph)
        gat_feature_4 = gat_feature_4[:, :max_seq_len, :]

        lstm_feature = self.hidden2hidden(lstm_feature)
        if self.strategy == "m":  #
            crf_feature = torch.cat((lstm_feature, gat_feature_1, gat_feature_2, gat_feature_3, gat_feature_4), dim=2)
            ...
            crf_feature = torch.mul(lstm_feature, self.weight1) + torch.mul(gat_feature_1, self.weight2) + torch.mul(
                gat_feature_2, self.weight3) + torch.mul(gat_feature_3, self.weight4) + torch.mul(gat_feature_4,
                                                                                                  self.weight5)
        return crf_feature

    def neg_log_likelihood(self, batch_char, batch_biword, batch_len, gaz_list, b_graph, c_graph, l_graph, s_graph,
                           mask, batch_label, sdp_list):
        crf_feature = self._get_crf_feature(batch_char, batch_biword, batch_len, gaz_list, b_graph, c_graph, l_graph,
                                            s_graph, sdp_list)
        total_loss = self.crf.neg_log_likelihood_loss(crf_feature, mask, batch_label)
        return total_loss

    def forward(self, batch_char, batch_biword, batch_len, gaz_list, b_graph, c_graph, l_graph, s_graph, mask,
                sdp_list):
        crf_feature = self._get_crf_feature(batch_char, batch_biword, batch_len, gaz_list, b_graph, c_graph, l_graph,
                                            s_graph, sdp_list)
