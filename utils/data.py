...
class Data:
    def __init__(self):
        ...
        self.gaz = Gazetteer(self.gaz_lower) # 预处理的词典，词典树
        ...
        # fzf 依存句法分析、 语义依存分析
        self.dep_train_texts = []
        self.dep_train_heads = []
        self.sdp_train_texts = []
        self.sdp_train_heads = []
        self.dep_dev_texts = []
        self.dep_dev_heads = []
        self.sdp_dev_texts = []
        self.dep_test_texts = []
    ...
    def show_data_summary(self):
        ...
        sys.stdout.flush()
    # 编译预训练的向量表 sgns.merge.word是300维的
    def build_gaz_file(self, gaz_file, skip_first_row=False, separator=" "):
        ## build gaz file,initial read gaz embedding file 初始化gaz向量文件
        if gaz_file:
    ...
    # fzf 双词结构
    def build_biword_pretrain_emb(self, emb_path, skip_first_row=False, separator=" "):
        print("build biword pretrain emb...")
        self.pretrain_biword_embedding, self.biword_emb_dim = \
            build_pretrain_embedding(emb_path, self.biword_alphabet, skip_first_row, separator,
                                                                                   self.biword_emb_dim,
    ...
    def build_instance(self, input_file, name, random_split=False):
        texts, ids, cut_num = read_instance(input_file, self.gaz, self.char_alphabet, self.biword_alphabet,
                                            self.label_alphabet, self.gaz_alphabet, self.number_normalized,
                                            self.max_sentence_length)
        if name == "train":
            if random_split:  # 如果需要将训练集分割一部分做验证集
                random.seed(1)
